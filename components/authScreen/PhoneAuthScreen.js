...
const PhoneAuthScreen = ({ navigation }) => {
  ...
  const buttonPress = async () => {
    try {
      const { data } = await publicAxios.post("connexion", {
  ...
  const sendSMS = async (phoneNumber) => {
    ...
    */
    ...
    if (isValidPhoneNumber(phoneNumber)) {
      try {
        const { data } = await publicAxios.post("/phone-activation-code", {
          phoneIndex: "+33",
          phone: phoneNumber,
        });
    ...
    <View style={styles.wrapper}>
      <View style={styles.secureText1}>
        ...
        <Text style={{ color: "white" }}>
          Sur ZEEVEN, pas d'arnaque : nous nous assurons de l'accès aux
          evènements a nos invités.
        </Text>
      </View>
      ...
      <Text style={TEXT_WHITE}>
        Ne t'en fais pas, cette info n'apparaitra pas sur ton profile et ne sera
